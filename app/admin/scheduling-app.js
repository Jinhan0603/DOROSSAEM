...
function submitAddScheduleRequest() {
    ...
    DB.lectureRequests.add({
        ...
        specialty_tags: tags,
        notes: document.getElementById('m-sr-notes').value
    });
...
function submitGenerateSessions() {
    ...
    document.querySelectorAll('.ses-date').forEach((el, i) => {
        ...
        dates.push({ date: el.value, start_time: start, end_time: end, duration_hours: endH - startH });
    });
...
function renderScheduleAssign(filterReqId) {
    ...
    if (filterReqId) {
        sessions = sessions.filter(s => s.request_id === filterReqId);
    }
...
function confirmAssignment(id) {
    ...
    toast('배정 확정됨', 'success');
    renderScheduleAssign();
}

function cancelAssignment(id) {
    DB.assignments.cancel(id, '매니저 취소');
    toast('배정 취소됨', 'warning');
    renderScheduleAssign();
}
...
function renderAvailability() {
    ...
    container.innerHTML = `
        <div class="card mb-3">
            <div class="form-row">
                <div class="form-group" style="flex:1;">
                    <label class="form-label">강사 선택</label>
                    <select class="form-select" id="avail-instructor" onchange="renderAvailabilityGrid()">
                        <option value="">선택</option>${insOptions}
                    </select>
                </div>
                <div class="form-group" style="flex:1;">
                    <label class="form-label">활동 지역</label>
                    <input type="text" class="form-input" id="avail-region" placeholder="예: 안산">
                </div>
                <div class="form-group" style="flex:1;">
                    <label class="form-label">이동수단</label>
                    <select class="form-select" id="avail-transport">
        ...
        <div style="margin-top:12px;">
            <button class="btn btn-primary" onclick="saveAvailability()"><i class="ph ph-floppy-disk"></i> 저장</button>
        </div>`;
}
...
function saveAvailability() {
    ...
    document.querySelectorAll('.avail-check').forEach(cb => {
        weekly_slots[cb.dataset.slot] = cb.checked;
    });
...
function renderLedger() {
    ...
    tbody.innerHTML = ledger.map(l => {
        ...
        const statusBadge = l.payback_status === 'completed' ? '<span class="badge active">완료</span>'
            : payback === 0 ? '<span class="badge">해당없음</span>'
        ...
        return `<tr>
            ...
            <td>${l.headcount}</td>
            <td>${l.lead_names || '-'}</td>
            <td>${l.assistant_names || '-'}</td>
            <td>${l.duration_hours}h</td>
            <td>₩${leadCost}</td>
            <td>₩${asstCost}</td>
            <td ${paybackClass}>${payback !== 0 ? '₩' + payback.toLocaleString() : '-'} ${statusBadge}</td>
        </tr>`;
    }).join('');
}
...
function exportLedgerCSV() {
    ...
    a.href = url;
    a.download = `doro_schedule_ledger_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
